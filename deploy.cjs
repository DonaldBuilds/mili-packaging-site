...
const handleConfigTest=async function(env){
  ...
  if(env.GA4_SERVICE_JSON&&env.GA4_PROPERTY_ID){
    try{const t=await oauthToken(env.GA4_SERVICE_JSON);const dr1=ga4DateRange(1);await ga4Report(t,env.GA4_PROPERTY_ID,'runReport',{dateRanges:[{startDate:dr1.start,endDate:dr1.end}],metrics:[{name:'activeUsers'}]});out.ga4='ok'}catch(e){out.ga4='fail:'+String((e&&e.message)||e)+'|len:'+(env.GA4_SERVICE_JSON||'').length+'|pk:'+(()=>{try{return (JSON.parse(env.GA4_SERVICE_JSON).private_key||'').length}catch(x){return 'parse:'+x.message}})()+'|der:'+(()=>{try{const pem=JSON.parse(env.GA4_SERVICE_JSON).private_key;const body=pem.replace(/-----BEGIN [A-Z ]*KEY-----/g,'').replace(/-----END [A-Z ]*KEY-----/g,'').replace(/\\n/g,'').replace(/\\s/g,'');const bin=atob(body);const arr=new Uint8Array(bin.length);for(let i=0;i<bin.length;i++)arr[i]=bin.charCodeAt(i);return 'body:'+body.length+','+arr.length+':'+Array.from(arr.slice(0,6)).map(b=>b.toString(16).padStart(2,'0')).join('')+'|pem:'+JSON.stringify(JSON.parse(env.GA4_SERVICE_JSON).private_key.slice(0,90))}catch(x){return 'err:'+x.message}})()}
