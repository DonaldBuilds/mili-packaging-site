...
// Discover hashed asset names dynamically (build output changes every build)
const assetJs = fs.readdirSync('./dist-seo/assets').find(f => f.startsWith('index-') && f.endsWith('.js'));
const assetCss = fs.readdirSync('./dist-seo/assets').find(f => f.startsWith('index-') && f.endsWith('.css'));
...
await check('SPA route /products -> fallback', 'https://mili-packaging.com/products', 200, 'text/html');
